...
import { ACCOUNTS_LINKS, CURRENCIES, LANGUAGES } from "../data/navbarData.js";
...
const mobileNavLinkClassName = ({ isActive }) =>
  `flex items-center gap-2 rounded-lg px-3 py-2.5 font-Poppins text-base font-medium transition-colors ${
    isActive
...
const Navbar = () => {
  ...
  const { user, isAuthenticated, logout, dashboardLink } = useAuth();
  const {
    cartItems,
    wishlistItems,
    cartCount,
  ...
  const accountLinks = useMemo(() => {
    ...
    return links;
  }, [isAuthenticated, user, logout, dashboardLink, navigate]);
  ...
  const openSecurePanel = useCallback(
    (panel, intent) => {
      if (!requireAuth(intent)) return;
      openPanel(panel);
    },
    [requireAuth, openPanel],
  );
  ...
  return (
    <header className="font-Poppins">
        ...
        <div className="page-container flex items-center justify-between gap-4 py-3 sm:py-4">
          ...
          <div className="flex shrink-0 items-center gap-3 sm:gap-4">
            ...
            <IconActionButton
              ...
              onClick={() => openSecurePanel("wishlist", "wishlist")}
            ...
            <IconActionButton
              label="Cart"
              count={isAuthenticated ? cartCount : 0}
              onClick={() => openSecurePanel("cart", "cart")}
              className="hidden sm:block"
              icon={
                <HugeiconsIcon
            ...
            <button
              ...
              className="rounded-lg p-1 text-primary transition-colors hover:bg-neutral-50 xl:hidden"
            >
              {isMenuOpen ? (
      ...
      <SearchOverlay isOpen={activePanel === "search"} onClose={closePanel} />
      {isAuthenticated && (
        <>
          <WishlistDrawer
